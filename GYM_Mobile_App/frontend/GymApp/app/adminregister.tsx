...
export default function AdminRegisterScreen() {
  ...
  const validateForm = (): boolean => {
    if (!adminName.trim()) {
      showPopup('Validation Error', 'Please enter the admin\'s full name.', 'error');
      return false;
    }
    if (!adminAge.trim()) {
      showPopup('Validation Error', 'Please enter the admin\'s age.', 'error');
      return false;
    }
    if (!adminNIC.trim()) {
      showPopup('Validation Error', 'Please enter the admin\'s NIC number.', 'error');
      return false;
    }
    if (!adminContactNumber.trim()) {
      showPopup('Validation Error', 'Please enter the admin\'s contact number.', 'error');
      return false;
    }
    if (adminContactNumber.trim().length !== 10) {
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/admin/admin-register`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          AdminDP: uploadedImageUrl || 'None',
          AdminName: adminName.trim(),
          AdminAge: adminAge.trim(),
          AdminNIC: adminNIC.trim(),
      ...
      if (response.status === 201) {
        showPopup('Success', 'Admin registered successfully', 'success');
      } else {
  ...
  return (
    <View style={styles.screen}>
      ...
      >
        ...
        {/* ── Page Title ── */}
        <Text style={styles.pageTitle}>Admin Registration</Text>
        ...
        <View style={styles.formContainer}>
          ...
          <View style={styles.inputGroup}>
            ...
            <View style={styles.inputWrapper}>
              ...
              <TextInput
                style={styles.input}
                placeholder="Enter admin's NIC number"
                placeholderTextColor={TEXT_MUTED}
                autoCapitalize="sentences"
                value={adminNIC}
                onChangeText={setAdminNIC}
              />
            </View>
          </View>
          ...
          <View style={styles.inputGroup}>
            ...
            <View style={styles.inputWrapper}>
              ...
              <TextInput
                ...
                onChangeText={setAdminContactNumber}
              />
            </View>
          </View>
