...
export default function CoachProfileScreen() {
  ...
  const coachId = (params.userId as string) || Session.getUserId();
  ...
  /* ── Fetch Coach Details ── */
  const fetchCoachDetails = async (id: string | null) => {
    if (!id) return;
    setIsLoading(true);
    try {
      const response = await fetch(`${BACKEND_URL}/api/coach/coach-details/${id}`);
      const data = await response.json();
      if (response.status === 200 && data.coach) {
        setCoachDetails(data.coach);
      } else {
        showAlert('Error', data.message || 'Failed to retrieve coach details.', 'error');
      }
    } catch (error: any) {
  ...
  const uploadProfileImage = async (uri: string, base64Data?: string) => {
    ...
    try {
      ...
      if (data.secure_url) {
        const updateResponse = await fetch(`${BACKEND_URL}/api/coach/coach-update-dp/${coachId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ CoachDP: data.secure_url }),
        });
  ...
  const handleUpdateContactNumber = async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/coach/coach-update-contact-number/${coachId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleChangePassword = async () => {
    ...
    try {
      // NOTE: Coach update password expects "ConfirmNewPassword" (capital C) in CoachController.js!
      const response = await fetch(`${BACKEND_URL}/api/coach/coach-update-password/${coachId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          oldPassword,
  ...
  const handleDeletePasswordSubmit = async () => {
    if (!coachId || !coachDetails) return;
    ...
    try {
      // Validate password via Coach Login endpoint
      const checkResponse = await fetch(`${BACKEND_URL}/api/coach/coach-login`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleConfirmDeleteAccount = async () => {
    ...
    setIsLoading(true);
    try {
      // NOTE: Coach delete expects "Password" (capital P) in CoachController.js!
      const response = await fetch(`${BACKEND_URL}/api/coach/coach-delete/${coachId}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
  ...
  const hasProfileImage = coachDetails?.CoachDP && coachDetails.CoachDP !== 'None' && coachDetails.CoachDP !== '';
  ...
  return (
    <View style={styles.screen}>
      ...
      {/* ── Hamburger Menu ── */}
      <HamburgerMenu currentRole="Coach" />
        ...
        >
          ...
          >
            <View style={styles.profileOuterWrapper}>
              <View style={styles.profileImageWrapper}>
                {hasProfileImage ? (
                  <Image source={{ uri: coachDetails.CoachDP }} style={styles.profileImage} />
                ) : (
          ...
          <View style={styles.greetingContainer}>
            <Text style={styles.greetingText}>
              Hi Coach {coachDetails?.CoachName || 'Coach'}!
            </Text>
          </View>

          {/* ── Coach Information Details Card ── */}
          <View style={styles.detailsCard}>
            <View style={styles.detailRow}>
              <Ionicons name="person-outline" size={20} color={TEXT_MUTED} />
              <View style={styles.detailContent}>
                <Text style={styles.detailLabel}>Coach Name</Text>
                <Text style={styles.detailValue}>{coachDetails?.CoachName || 'N/A'}</Text>
              </View>
            </View>
            ...
            <View style={styles.detailRow}>
              ...
              <View style={styles.detailContent}>
                ...
                <Text style={styles.detailValue}>{coachDetails?.Email || 'N/A'}</Text>
              </View>
            </View>
            ...
            <View style={styles.detailRow}>
              ...
              <View style={styles.detailContent}>
                ...
                <Text style={styles.detailValue}>{coachDetails?.CoachNIC || 'N/A'}</Text>
              </View>
            </View>
            ...
            <View style={styles.detailRow}>
              ...
              <View style={styles.detailContent}>
                ...
                <Text style={styles.detailValue}>{coachDetails?.CoachID || 'N/A'}</Text>
              </View>
            </View>
            ...
            <View style={styles.detailRow}>
              ...
              <View style={styles.detailContent}>
                ...
                <Text style={styles.detailValue}>{coachDetails?.CoachContactNumber || 'N/A'}</Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.buttonsRow}>
            <TouchableOpacity
              ...
              onPress={() => {
                setNewContactNumber(coachDetails?.CoachContactNumber || '');
      ...
      <Modal
        ...
        transparent={true}
        animationType="fade"
        onRequestClose={passwordModalVisible => setPasswordModalVisible(false)}
      >
        <View style={styles.modalOverlay}>
          <View style={styles.formModalCard}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.formModalCard}>
            <Text style={styles.modalConfirmText}>
              Are you sure you want to permanently delete this coach account?
            </Text>
