...
export default function GymManagementScreen() {
  ...
  /* ── Fetch Data ── */
  const fetchGyms = async (showLoading = true) => {
    if (showLoading) setIsLoading(true);
    try {
      // Fetch gyms sorted with Approve = false at the top
      const response = await fetch(`${BACKEND_URL}/api/admin-manage/admin-manage-sort-up-to-approve-fales-gyms`);
      const data = await response.json();
      if (response.status === 200 && data.gyms) {
        setGyms(data.gyms);
      } else {
        showAlert('Error', data.message || 'Failed to retrieve gyms list.', 'error');
      }
    } catch (error) {
  ...
  /* ── Approve / Reject Gym ── */
  const handleToggleApprove = async (targetGymId: string, newStatus: boolean) => {
    ...
    try {
      const response = await fetch(
        `${BACKEND_URL}/api/admin-manage/admin-manage-change-approve-for-gym/${targetGymId}/${loginAdminId}`,
        {
          method: 'PATCH',
      ...
      if (response.status === 200) {
        // Refresh local items
        await fetchGyms(false);
        showAlert(
          'Success',
          `Gym has been ${newStatus ? 'Approved' : 'Rejected'} successfully.`,
          'success'
        );
      } else {
  ...
  const openDeleteConfirm = (gym: Gym) => {
    ...
    setDeleteConfirm({ visible: true, targetGym: gym });
  };

  const closeDeleteConfirm = () => {
    setDeleteConfirm({ visible: false, targetGym: null });
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    const targetGymId = deleteConfirm.targetGym?._id;
    if (!targetGymId || !loginAdminId) return;

    closeDeleteConfirm();
    setIsActionLoading(true);
    try {
      const response = await fetch(
        `${BACKEND_URL}/api/admin-manage/admin-manage-delete-gym/${targetGymId}/${loginAdminId}`,
        {
          method: 'DELETE',
        }
      );
      ...
      if (response.status === 200) {
        // Refresh local items
        await fetchGyms(false);
        showAlert('Success', 'Gym has been permanently deleted.', 'success');
      } else {
        showAlert('Deletion Failed', data.message || 'Failed to delete gym.', 'error');
      }
    } catch (error) {
      showAlert('Network Error', 'Could not delete gym.', 'error');
    } finally {
      setIsActionLoading(false);
    }
  };
  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={styles.headerContainer}>
        ...
        {/* Title */}
        <Text style={styles.pageTitle}>Gym Management</Text>
      ...
      >
        {isLoading ? (
          ...
          <View style={styles.emptyContainer}>
            <Ionicons name="barbell-outline" size={64} color={TEXT_MUTED} />
            <Text style={styles.emptyText}>No gyms registered</Text>
            <Text style={styles.emptySubtext}>There are currently no gym accounts in the system.</Text>
          </View>
        ) : (
          <View style={styles.cardsContainer}>
            {gyms.map((item) => {
              const hasLogo = item.GymLogo && item.GymLogo !== 'None' && item.GymLogo !== '';
              return (
                <View key={item._id} style={styles.card}>
                  {/* Card Header: Logo, Name, Type, Status */}
                  <View style={styles.cardHeader}>
                    <View style={styles.avatarWrapper}>
                      {hasLogo ? (
                        <Image source={{ uri: item.GymLogo as string }} style={styles.avatar} />
                      ) : (
                        <View style={styles.avatarPlaceholder}>
                          <Ionicons name="barbell-outline" size={20} color={TEXT_SECONDARY} />
                        </View>
                      )}
                    </View>
                  ...
                  <View style={styles.cardBody}>
                    ...
                    <View style={styles.infoRow}>
                      ...
                      <Text style={styles.infoValue} numberOfLines={2}>{item.GymAddress}</Text>
                    </View>
                  </View>
                  ...
                  <View style={styles.cardBody}>
                    ...
                    <View style={styles.infoRow}>
                      ...
                      <Text style={styles.infoValue}>{item.GymOwnerContactNumber}</Text>
                    </View>

                    <View style={styles.infoRow}>
                      <Ionicons name="card-outline" size={15} color={TEXT_MUTED} />
                      <Text style={styles.infoValue}>Owner NIC: {item.GymOwnerNIC}</Text>
                    </View>
                  </View>
                  ...
                  {isAdminApproved === null ? (
                    ...
                    <View style={[styles.actionsContainer, isAdminApproved === false && { opacity: 0.5 }]}>
                      ...
                      >
                        ...
                        <Text style={styles.deleteGymButtonText}>Delete Gym</Text>
                      </TouchableOpacity>
                    </View>
                  )}
                </View>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.confirmModalCard}>
            ...
            <Text style={styles.confirmTitle}>Delete Gym</Text>
            <Text style={styles.modalConfirmText}>
              Are you sure you want to permanently delete gym "{deleteConfirm.targetGym?.GymName}"? This action cannot be undone.
            </Text>
