...
export default function GymProfileScreen() {
  ...
  const gymId = (params.userId as string) || Session.getUserId();
  ...
  /* ── Fetch Gym Details ── */
  const fetchGymDetails = async (id: string | null) => {
    if (!id) return;
    setIsLoading(true);
    try {
      const response = await fetch(`${BACKEND_URL}/api/gym/gym-details/${id}`);
      const data = await response.json();
      if (response.status === 200 && data.gym) {
        setGymDetails(data.gym);
      } else {
        showAlert('Error', data.message || 'Failed to retrieve gym details.', 'error');
      }
    } catch (error: any) {
  ...
  const selectGymLogo = async () => {
    ...
    if (!result.canceled && result.assets[0]) {
      ...
      await uploadGymLogo(asset.uri, asset.base64 || undefined);
    }
  };

  const uploadGymLogo = async (uri: string, base64Data?: string) => {
    setIsUploadingLogo(true);
    try {
      const formData = new FormData();
      formData.append('file', base64Data ? `data:image/jpeg;base64,${base64Data}` : {
        uri,
        name: `gymlogo_${Date.now()}.jpg`,
        type: 'image/jpeg',
      } as any);
      ...
      if (data.secure_url) {
        const updateResponse = await fetch(`${BACKEND_URL}/api/gym/gym-update-gym-logo/${gymId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ gymLogo: data.secure_url }),
        });

        const updateData = await updateResponse.json();

        if (updateResponse.status === 200) {
          showAlert('Success', 'Gym logo updated successfully.', 'success');
          fetchGymDetails(gymId);
        } else {
          showAlert('Update Failed', updateData.message || 'Failed to save gym logo.', 'error');
        }
      } else {
      ...
      showAlert('Upload Error', error.message || 'An error occurred during logo upload.', 'error');
    } finally {
  ...
  const handleUpdateContactNumber = async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/gym/gym-update-contact-number/${gymId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleChangePassword = async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/gym/gym-update-password/${gymId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          oldPassword,
  ...
  const handleDeletePasswordSubmit = async () => {
    ...
    try {
      // Validate password via Gym Login endpoint
      const checkResponse = await fetch(`${BACKEND_URL}/api/gym/gym-login`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleConfirmDeleteAccount = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${BACKEND_URL}/api/gym/gym-delete/${gymId}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
  ...
  const hasLogo = gymDetails?.GymLogo && gymDetails.GymLogo !== 'None' && gymDetails.GymLogo !== '';
        ...
        >
          ...
          >
            <View style={styles.profileOuterWrapper}>
              <View style={styles.profileImageWrapper}>
                {hasLogo ? (
                  <Image source={{ uri: gymDetails.GymLogo }} style={styles.profileImage} />
                ) : (
            ...
            <Text style={styles.profileHint}>
              {isUploadingLogo ? 'Uploading...' : 'Tap to change logo'}
            </Text>
          </TouchableOpacity>
          ...
          <View style={styles.greetingContainer}>
            <Text style={styles.greetingText}>
              Hi {gymDetails?.GymName || 'Gym'} Gym!
            </Text>
          </View>

          {/* ── Gym Information Details Card ── */}
          <View style={styles.detailsCard}>
            <View style={styles.detailRow}>
              <Ionicons name="business-outline" size={20} color={TEXT_MUTED} />
              <View style={styles.detailContent}>
                <Text style={styles.detailLabel}>Gym Name</Text>
                <Text style={styles.detailValue}>{gymDetails?.GymName || 'N/A'}</Text>
              </View>
            </View>
            ...
            <View style={styles.detailRow}>
              ...
              <View style={styles.detailContent}>
                ...
                <Text style={styles.detailValue}>{gymDetails?.Email || 'N/A'}</Text>
              </View>
            </View>
            ...
            <View style={styles.detailRow}>
              <Ionicons name="key-outline" size={20} color={TEXT_MUTED} />
              <View style={styles.detailContent}>
                <Text style={styles.detailLabel}>Gym ID</Text>
                <Text style={styles.detailValue}>{gymDetails?.GymID || 'N/A'}</Text>
              </View>
            </View>
            ...
            <View style={styles.detailRow}>
              ...
              <View style={styles.detailContent}>
                ...
                <Text style={styles.detailValue}>{gymDetails?.GymOwnerName || 'N/A'}</Text>
              </View>
            </View>
            ...
            <View style={styles.detailRow}>
              ...
              <View style={styles.detailContent}>
                <Text style={styles.detailLabel}>Owner NIC Card</Text>
                <Text style={styles.detailValue}>{gymDetails?.GymOwnerNIC || 'N/A'}</Text>
              </View>
            </View>
            ...
            <View style={styles.detailRow}>
              ...
              <View style={styles.detailContent}>
                ...
                <Text style={styles.detailValue}>{gymDetails?.GymOwnerContactNumber || 'N/A'}</Text>
              </View>
            </View>
            ...
            <View style={styles.detailRow}>
              ...
              <View style={styles.detailContent}>
                <Text style={styles.detailLabel}>Gym Address</Text>
                <Text style={styles.detailValue}>{gymDetails?.GymAddress || 'N/A'}</Text>
              </View>
            </View>
            ...
            <View style={styles.detailRow}>
              ...
              <View style={styles.detailContent}>
                ...
                <Text style={styles.detailValue}>{gymDetails?.GymType || 'N/A'}</Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.buttonsRow}>
            <TouchableOpacity
              ...
              onPress={() => {
                setNewContactNumber(gymDetails?.GymOwnerContactNumber || '');
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.formModalCard}>
            <Text style={styles.modalConfirmText}>
              Are you sure you want to permanently delete this gym account?
            </Text>
