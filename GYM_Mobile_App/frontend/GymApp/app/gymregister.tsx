...
export default function GymRegisterScreen() {
  ...
  const [gymOwnerNIC, setGymOwnerNIC] = useState('');
  const [gymID, setGymID] = useState('');
  const [gymAddress, setGymAddress] = useState('');
  const [gymOwnerContactNumber, setGymOwnerContactNumber] = useState('');
  ...
  const pickImage = async () => {
    ...
    if (!permissionResult.granted) {
      showPopup('Permission Required', 'Please allow access to your photo library to select a gym logo.', 'error');
      return;
    }
  ...
  const validateForm = (): boolean => {
    if (!gymName.trim()) {
      showPopup('Validation Error', 'Please enter the gym name.', 'error');
      return false;
    }
    if (!gymOwnerName.trim()) {
      showPopup('Validation Error', 'Please enter the gym owner\'s full name.', 'error');
      return false;
    }
    if (!gymOwnerNIC.trim()) {
      showPopup('Validation Error', 'Please enter the gym owner\'s NIC number.', 'error');
      return false;
    }
    if (!gymID.trim()) {
      showPopup('Validation Error', 'Please enter the Gym ID.', 'error');
      return false;
    }
    if (!gymAddress.trim()) {
      showPopup('Validation Error', 'Please enter the gym address.', 'error');
      return false;
    }
    if (!gymOwnerContactNumber.trim()) {
      showPopup('Validation Error', 'Please enter the gym owner\'s contact number.', 'error');
      return false;
    }
    if (gymOwnerContactNumber.trim().length !== 10) {
      showPopup('Validation Error', 'Contact number must be 10 digits.', 'error');
      return false;
    }
    if (!gymType.trim()) {
      showPopup('Validation Error', 'Please enter the gym type.', 'error');
      return false;
    }
    if (!email.trim()) {
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/gym/gym-register`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          GymLogo: uploadedLogoUrl || 'None',
          GymName: gymName.trim(),
          GymOwnerName: gymOwnerName.trim(),
          GymOwnerNIC: gymOwnerNIC.trim(),
          GymID: gymID.trim(),
          GymAddress: gymAddress.trim(),
          GymOwnerContactNumber: gymOwnerContactNumber.trim(),
      ...
      if (response.status === 201) {
        showPopup('Success', 'Gym registered successfully', 'success');
      } else {
  ...
  return (
    <View style={styles.screen}>
      ...
      >
        ...
        {/* ── Page Title ── */}
        <Text style={styles.pageTitle}>Gym Registration</Text>
        ...
        >
          <View style={styles.profileOuterWrapper}>
            <View style={styles.profileImageWrapper}>
              {gymLogo ? (
                <Image source={{ uri: gymLogo }} style={styles.profileImage} />
              ) : (
          ...
          <Text style={styles.profileHint}>
            {isUploading ? 'Uploading...' : 'Tap to add gym logo'}
          </Text>

          {/* Upload Status */}
          {uploadedLogoUrl !== '' && !isUploading && (
            <View style={styles.uploadSuccess}>
              <Ionicons name="checkmark-circle" size={16} color={SUCCESS_GREEN} />
              <Text style={styles.uploadSuccessText}>Logo uploaded</Text>
            </View>
          )}
        </TouchableOpacity>
        ...
        <View style={styles.formContainer}>
          ...
          {/* Gym Owner Full Name */}
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Gym Owner Full Name</Text>
            <View style={styles.inputWrapper}>
              <Ionicons name="person-outline" size={20} color={TEXT_MUTED} style={styles.inputIcon} />
              <TextInput
                style={styles.input}
                placeholder="Enter gym owner's full name"
                placeholderTextColor={TEXT_MUTED}
                autoCapitalize="words"
                value={gymOwnerName}
                onChangeText={setGymOwnerName}
              />
            </View>
          </View>
          ...
          {/* Gym Owner NIC */}
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Gym Owner NIC</Text>
            <View style={styles.inputWrapper}>
              <Ionicons name="card-outline" size={20} color={TEXT_MUTED} style={styles.inputIcon} />
              <TextInput
                style={styles.input}
                placeholder="Enter gym owner's NIC number"
                placeholderTextColor={TEXT_MUTED}
                autoCapitalize="sentences"
                value={gymOwnerNIC}
                onChangeText={setGymOwnerNIC}
              />
            </View>
          </View>
          ...
          {/* Gym ID */}
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Gym ID</Text>
            <View style={styles.inputWrapper}>
          ...
          {/* Gym Address */}
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Gym Address</Text>
            <View style={styles.inputWrapper}>
          ...
          {/* Gym Owner Contact Number */}
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Gym Owner Contact Number</Text>
            <View style={styles.inputWrapper}>
              ...
              <TextInput
                ...
                maxLength={10}
                value={gymOwnerContactNumber}
                onChangeText={setGymOwnerContactNumber}
              />
            </View>
          </View>
