...
export default function UserProfileScreen() {
  ...
  const userId = (params.userId as string) || Session.getUserId();
  ...
  /* ── Fetch User Details ── */
  const fetchUserDetails = async (id: string | null) => {
    if (!id) return;
    setIsLoading(true);
    try {
      const response = await fetch(`${BACKEND_URL}/api/user/user-details/${id}`);
      const data = await response.json();
      if (response.status === 200 && data.user) {
        setUserDetails(data.user);
      } else {
        showAlert('Error', data.message || 'Failed to retrieve user details.', 'error');
      }
    } catch (error: any) {
  ...
  const uploadProfileImage = async (uri: string, base64Data?: string) => {
    ...
    try {
      ...
      if (data.secure_url) {
        const updateResponse = await fetch(`${BACKEND_URL}/api/user/user-update-dp/${userId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ UserDP: data.secure_url }),
        });
      ...
      showAlert('Upload Error', error.message || 'An error occurred during image upload.', 'error');
    } finally {
  ...
  const handleUpdateContactNumber = async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/user/user-update-contactnumber/${userId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.status === 200) {
        ...
        // Backend returns "Contact number must be 10 degites"
  ...
  const handleChangePassword = async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/user/user-update-password/${userId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          oldPassword,
      ...
      if (response.status === 200) {
        ...
        // Handle backend returns like "Invalid old password", etc.
  ...
  const handleDeletePasswordSubmit = async () => {
    ...
    try {
      // Validate password via Login endpoint
      const checkResponse = await fetch(`${BACKEND_URL}/api/user/user-login`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (checkResponse.status === 200) {
        ...
        // Backend returns "Invalid password"
  ...
  const handleConfirmDeleteAccount = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${BACKEND_URL}/api/user/user-delete/${userId}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
  ...
  const hasProfileImage = userDetails?.UserDP && userDetails.UserDP !== 'None' && userDetails.UserDP !== '';
        ...
        >
          ...
          >
            <View style={styles.profileOuterWrapper}>
              <View style={styles.profileImageWrapper}>
                {hasProfileImage ? (
                  <Image source={{ uri: userDetails.UserDP }} style={styles.profileImage} />
                ) : (
          ...
          <View style={styles.detailsCard}>
            <View style={styles.detailRow}>
              ...
              <View style={styles.detailContent}>
                ...
                <Text style={styles.detailValue}>{userDetails?.UserName || 'N/A'}</Text>
              </View>
            </View>
            ...
            <View style={styles.detailRow}>
              ...
              <View style={styles.detailContent}>
                ...
                <Text style={styles.detailValue}>{userDetails?.UserNIC || 'N/A'}</Text>
              </View>
            </View>
            ...
            <View style={styles.detailRow}>
              ...
              <View style={styles.detailContent}>
                ...
                <Text style={styles.detailValue}>{userDetails?.UserContactNumber || 'N/A'}</Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.buttonsRow}>
            <TouchableOpacity
              ...
              onPress={() => {
                setNewContactNumber(userDetails?.UserContactNumber || '');
